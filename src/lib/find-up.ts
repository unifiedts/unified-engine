...
function isCallback<Value>(value: unknown[]): value is Array<Callback<Value>>;
function isCallback<Value>(value: unknown): value is Callback<Value>;
function isCallback<Value>(
	value: unknown | unknown[],
): value is Callback<Value> | Array<Callback<Value>> {
	return typeof value === 'function';
}

export class FindUp<Value> {
	cache: Record<string, Array<Callback<Value>> | undefined | Error | Value>;
	cwd: string;
	detect: boolean;
	names: string[];
	create: Create<Value>;
	givenFilePath?: string | undefined;
	givenFile: Error | Value | Array<Callback<Value>> | undefined;
	...
	load(filePath: string, callback: Callback<Value>) {
		...
		function find(directory: string) {
			...
			function found(error: Error | null, result?: Value): void {
				...
				if (!Array.isArray(cbs)) {
					throw new TypeError(
						`Expected cache to contain an array of callback`,
					);
				}
		...
		function apply(
			cb: Callback<Value>,
			result: Value | Error | Array<Callback<Value>> | undefined,
		) {
